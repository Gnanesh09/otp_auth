...
import { Phone, Lock, Eye, EyeOff, CheckCircle, XCircle, RefreshCw, Trash2 } from 'lucide-react';
...
const OTPVerification = () => {
  ...
  const [showOtp, setShowOtp] = useState(true); // Demo toggle
  const [allOtps, setAllOtps] = useState({});
  const [backendStatus, setBackendStatus] = useState('connecting'); // 'connected', 'error', 'connecting'

  const API_BASE = 'https://apiotpv1.vercel.app'
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setMessage('Network error. Please check if backend is running on http://localhost:8000');
      setMessageType('error');
    } finally {
      setLoading(false);
      fetchAllOtps();
  ...
  const clearOtp = async (phone) => {
    try {
      await fetch(`${API_BASE}/clear-otp/${phone}`, {
        method: 'DELETE',
      });
      fetchAllOtps();
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Secure phone number verification system with Vite + React</p>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            {message && (
              <div className={`p-4 rounded-lg mb-6 flex items-center transition-all duration-300 ${
                messageType === 'success' 
                ...
                {generatedOtp && showOtp && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-yellow-800">Demo OTP:</span>
                      <button
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              >
                {showOtp ? <Eye className="w-4 h-4 mr-2" /> : <EyeOff className="w-4 h-4 mr-2" />}
            ...
            <div className="space-y-4">
                ...
                {Object.keys(allOtps).length === 0 ? (
                  ...
                  <div className="space-y-3 max-h-64 overflow-y-auto">
                    {Object.entries(allOtps).map(([phone, data]) => (
                      <div key={phone} className="bg-white rounded-lg p-3 border hover:shadow-sm transition-shadow">
                        <div className="flex items-center justify-between mb-2">
              ...
              {/* <div className="bg-blue-50 rounded-lg p-4">
                ...
                <ul className="text-sm text-blue-700 space-y-1">
                  ...
                  <li>• Real-time OTP status updates</li>
                  <li>• Toggle OTP visibility for demo</li>
                  <li>• Clear individual OTPs</li>
                  <li>• Built with Vite + React</li>
                </ul>
              </div>

              <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="font-medium text-gray-800 mb-2">Quick Test</h4>
                <p className="text-sm text-gray-600 mb-3">
                  Use any phone number to generate an OTP, then use the visible OTP code to verify.
                </p>
                <div className="text-xs text-gray-500">
                  <p>Backend: FastAPI (Port 8000)</p>
                  <p>Frontend: Vite + React (Port 5173)</p>
                </div>
              </div> */}
            </div>
          </div>
